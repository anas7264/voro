...
## 2026-06-14 - Secure Authenticated Encrypted Backups with Inline-Style Visual Protection
**Vulnerability:** Raw local data stored in custom client backups can contain sensitive PII or health/biometric information. Exporting backups in plaintext JSON exposes this sensitive dataset. Furthermore, restoring raw JSON backups without whitelisting, prototype pollution protections, or input sanitization exposes the application to state/prototype corruption attacks.
**Learning:** Authenticated encryption (AES-GCM) with HKDF domain-specific keys ensures backup confidentiality and cryptographic integrity. Whitelisting keys and sanitizing input on import mitigates prototype pollution and state-injection vectors. To maintain design system fidelity and keyboard accessibility, hidden files inputs used to trigger imports are best hidden using absolute inline styles (`style={{ display: 'none' }}`) to prevent rendering style-race conditions or CSS-framework mismatches.
**Prevention:** Always encrypt exported backup archives using robust authenticated encryption, and validate all decrypted keys against an allowlist and prototype pollution guards before importing them.
