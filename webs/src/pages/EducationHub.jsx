...
/**
 * ⚡ PERFORMANCE OPTIMIZATION: Hoisted and frozen static datasets.
 * Prevents allocating arrays and object literals on every render.
 */
const RAW_ARTICLES = Object.freeze([
  {
    id: 1,
...
const FEATURED_ARTICLE = RAW_ARTICLES.find(a => a.featured);
...
/**
 * ⚡ SUBCOMPONENT: KineticEducationAlignmentOverlay
 * Cinematic 2.5s orbital alignment sequence with concentric telemetry rings,
 * Playwright E2E test bypass support, and smooth exit state.
 */
const KineticEducationAlignmentOverlay = memo(({ onComplete }) => {
  const [phase, setPhase] = useState(0);

  useEffect(() => {
    // E2E test bypass hook check
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#020408] flex flex-col items-center justify-center p-6 select-none animate-fade-in">
      {/* Dynamic Backglow */}
      <div className="absolute w-[600px] h-[600px] bg-voro-primary/10 rounded-full blur-[140px] pointer-events-none" />

      {/* Concentric Orbital Telemetry Ring */}
      <div className="relative w-48 h-48 mb-12 flex items-center justify-center">
        <div className="absolute inset-0 rounded-full border border-white/5 animate-[spin_10s_linear_infinite]" />
        <div className="absolute inset-2 rounded-full border border-dashed border-voro-primary/30 animate-[spin_15s_linear_infinite_reverse]" />
        <div className="absolute inset-6 rounded-full border border-voro-secondary/20 animate-[spin_8s_linear_infinite]" />

        <div className="relative z-10 flex flex-col items-center justify-center">
          <Newspaper size={36} className="text-voro-primary animate-pulse mb-2" />
          <span className="font-mono text-[0.5rem] text-gray-400 font-bold tracking-[0.3em] uppercase">INTEL_SYNC</span>
        </div>
      </div>

      {/* Telemetry Status Line */}
      <div className="space-y-3 text-center max-w-sm">
        <h3 className="font-serif italic text-2xl text-white font-medium tracking-tight">
          Synchronizing Intellectual Archive
        </h3>
        <div className="flex items-center justify-center gap-3 font-mono text-[0.6rem] text-voro-primary tracking-[0.3em] uppercase">
          <span className="w-1.5 h-1.5 rounded-full bg-voro-primary animate-ping" />
          <span>
            {phase === 0 && 'Indexing Biological Dossiers...'}
            {phase === 1 && 'Attesting Neural Provenance...'}
            {phase === 2 && 'Calibrating Kinetic Telemetry...'}
...
KineticEducationAlignmentOverlay.displayName = 'KineticEducationAlignmentOverlay';

/**
 * ⚡ SUBCOMPONENT: DossierHero
 * Features multi-layered parallax, volumetric 3D transforms, and industrial telemetry.
 * Utilizes direct DOM manipulation for 60fps performance.
 */
const DossierHero = memo(({ article, onAccessDossier }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    if (imageRef.current) {
      imageRef.current.style.transform = `scale(1.08) translate3d(${nx * 30}px, ${ny * 30}px, 0)`;
    }
    if (contentRef.current) {
      contentRef.current.style.transform = `translate3d(${nx * -20}px, ${ny * -20}px, 40px) rotateX(${ny * -10}deg) rotateY(${nx * 10}deg)`;
    }
  ...
  return (
    <section
      ...
      className="relative h-[720px] w-full overflow-hidden rounded-[4rem] border border-white/5 shadow-[0_80px_160px_-40px_rgba(0,0,0,0.8)] group/hero perspective-[2000px] mb-24"
    >
      ...
      <div
        ref={contentRef}
        className="absolute bottom-20 left-16 right-16 md:bottom-24 md:left-24 md:right-24 z-10 transition-transform duration-700 ease-out preserve-3d pointer-events-none"
      >
        <div className="max-w-5xl space-y-8">
          <div className="space-y-6">
            <div className="flex flex-wrap items-center gap-6">
              <Badge variant="voro-primary" dot className="px-6 py-2 font-black tracking-[0.4em] backdrop-blur-3xl bg-voro-primary/20 border-white/10 pointer-events-auto">
                Priority Dossier
              </Badge>
              <div className="flex items-center gap-3 text-gray-400 font-mono text-[0.7rem] uppercase tracking-[0.4em]">
                <Clock size={14} className="text-voro-primary animate-pulse" />
                <span>Archive Depth: {article.readTime}</span>
              </div>
            </div>

            <h2 className="text-4xl md:text-6xl lg:text-[6.5rem] font-serif italic font-medium text-white tracking-tighter leading-[0.85] drop-shadow-[0_20px_40px_rgba(0,0,0,0.8)]">
              {article.title}
            </h2>

            <p className="text-gray-300 text-lg md:text-xl font-light leading-relaxed max-w-4xl opacity-90 drop-shadow-lg">
              {article.excerpt}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-8 pt-4">
            <div className="flex items-center gap-5 pointer-events-auto group/author">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-[0.6rem] font-mono text-voro-primary/60 uppercase tracking-[0.4em]">{article.date} Publication</p>
              </div>
            </div>

            <button
              onClick={() => onAccessDossier?.(article.title)}
              className="group/btn relative flex items-center gap-4 px-12 py-5 bg-white text-black rounded-full text-[0.75rem] font-black uppercase tracking-[0.5em] transition-all duration-700 hover:scale-105 hover:shadow-[0_40px_80px_rgba(255,255,255,0.25)] active:scale-95 pointer-events-auto overflow-hidden focus-visible:scale-105 focus-visible:ring-2 focus-visible:ring-voro-primary focus-visible:ring-offset-2 focus-visible:ring-offset-[#020408] outline-none"
            >
              ...
              <ArrowUpRight size={18} className="relative z-10 group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 group-focus-visible/btn:translate-x-1 group-focus-visible/btn:-translate-y-1 transition-transform duration-500" />
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-16 right-16 pointer-events-none flex flex-col items-end gap-2 font-mono text-[0.5rem] font-black text-white/20 uppercase tracking-[0.5em]">
...
/**
 * ⚡ SUBCOMPONENT: EducationArticleCard
 * Re-engineered with 60fps direct-DOM 3D Volumetric Hover Tilts,
 * dynamic radial spotlight, reactive liquid perimeter illumination,
 * spatial coordinate telemetry overlay, and static 4-degree APG focus tilt.
 */
const EducationArticleCard = memo(({ article, idx, isBookmarked, onToggleBookmark, onShare, onReadDossier }) => {
  const containerRef = useRef(null);
  ...
  return (
    <div
      ...
      tabIndex="0"
      role="article"
      aria-label={`Research Dossier: ${article.title} by ${article.author}`}
      style={{
    ...
    >
      ...
      <div
        className="absolute top-5 right-6 pointer-events-none opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-all duration-500 z-30"
      ...
      >
        <div className="flex flex-col items-end font-mono text-[0.45rem] font-bold text-voro-primary/60 tracking-[0.2em] space-y-0.5">
          ...
          <span className="text-white/20">[0xEDU_ART_0{article.id}]</span>
        </div>
      </div>

      {/* Image Container */}
      <div className="relative h-64 overflow-hidden z-10" style={{ transform: 'translateZ(20px)' }}>
        <img
          src={article.image}
          alt={article.title}
      ...
      {/* Content Container */}
      <div className="p-8 flex-1 flex flex-col relative z-10" style={{ transform: 'translateZ(40px)' }}>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2 text-gray-500 font-mono text-[0.55rem] uppercase tracking-widest">
            <Clock size={12} className="text-voro-primary/70" />
        ...
        <h3 className="text-2xl font-serif italic font-medium text-white mb-4 leading-tight tracking-tight group-hover:text-voro-primary transition-colors duration-500">
          {article.title}
        </h3>
        ...
        <div className="pt-6 border-t border-white/5 flex items-center justify-between">
          ...
          >
            Read Dossier
            <ArrowUpRight size={12} className="group-hover/rd:translate-x-1 group-hover/rd:-translate-y-1 transition-transform duration-300" />
          </button>
        </div>
      </div>
    </div>
...
EducationArticleCard.displayName = 'EducationArticleCard';

/**
 * ⚡ MAIN COMPONENT: EducationHub
 * The flagship 'Intellectual Archive & Knowledge Synthesis' view.
 */
const EducationHub = () => {
  ...
  return (
    <>
      {loading && (
        <KineticEducationAlignmentOverlay onComplete={() => setLoading(false)} />
      )}
        ...
        <div className="relative max-w-[1440px] mx-auto px-6 py-12 md:px-12 lg:px-20">
          <header className="mb-20 flex flex-col md:flex-row md:items-end justify-between gap-10">
            <div className="space-y-4 max-w-2xl">
              <div className="flex items-center gap-3 text-voro-primary">
                <Newspaper size={18} />
                <span className="text-[0.6rem] font-mono font-bold uppercase tracking-[0.3em]">Knowledge Synthesis</span>
              </div>
              <h1 className="text-5xl md:text-7xl font-serif italic font-medium tracking-tight text-white leading-tight">
                Intellectual <span className="text-voro-primary not-italic font-bold">Archive</span>
              </h1>
              <p className="text-gray-500 font-mono font-medium tracking-widest text-[0.65rem] uppercase opacity-60 leading-relaxed">
              ...
              <div className="flex flex-wrap gap-2">
                {CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`px-5 py-2 rounded-full text-[0.55rem] font-black uppercase tracking-[0.2em] transition-all border focus-visible:ring-2 focus-visible:ring-voro-primary focus-visible:ring-offset-2 focus-visible:ring-offset-[#020408] outline-none ${
                      activeCategory === cat
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredArticles.map((article, idx) => (
              <EducationArticleCard
                ...
                onShare={handleShare}
                onReadDossier={handleReadDossier}
              />
            ))}
          </div>
            ...
            <div className="py-40 text-center space-y-8 animate-fade-in">
               <div className="w-24 h-24 mx-auto bg-white/[0.02] border border-white/5 rounded-full flex items-center justify-center shadow-inner">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[0.6rem] text-gray-600 uppercase font-mono font-black tracking-[0.3em]">Query returned zero biological artifacts</p>
                </div>
          ...
          <footer className="mt-32 pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-10">
             <div className="flex items-center gap-6">
                <span className="text-[0.6rem] font-mono font-black text-gray-600 uppercase tracking-[0.4em]">VORO Research Journal</span>
                <div className="w-1 h-1 rounded-full bg-gray-800" />
                <span className="text-[0.6rem] font-mono font-black text-gray-600 uppercase tracking-[0.4em]">Est. 2024</span>
             </div>
