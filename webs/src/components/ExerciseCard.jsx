...
export const ExerciseCard = memo(({
  ...
  const nodeId = useMemo(() => {
    ...
    return `EX_NODE_${cleanId.slice(0, 6).toUpperCase()}`;
  }, [reactId]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 rounded-[2.5rem] overflow-hidden pointer-events-none">
        <div className="absolute inset-0 bg-grid-white opacity-0 group-hover/ex:opacity-100 group-focus-visible:opacity-100 transition-opacity duration-1000" style={{ transform: 'translateZ(10px)' }} />
        <div className="absolute inset-0 bg-boutique-grain opacity-[0.02]" />

        {/* Dynamic Luminous Lens (Mouse Tracking) */}
        <div
          className="absolute inset-0 opacity-0 group-hover/ex:opacity-100 group-focus-visible:opacity-100 transition-opacity duration-700"
          style={{
      ...
      <div
        className="absolute top-6 right-8 pointer-events-none opacity-0 group-hover/ex:opacity-100 group-focus-visible:opacity-100 transition-all duration-500"
